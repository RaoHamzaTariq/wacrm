...
describe('retrieveKnowledge', () => {
  it('returns [] for an empty query without touching the DB', async () => {
    ...
    expect(await retrieveKnowledge(db, 'acct', { embeddingsApiKey: null, embeddingsBaseUrl: null }, '  ')).toEqual([])
  ...
  it('short-circuits (no embed, no RPC) when the KB is empty', async () => {
    const { db, state } = makeDb()
    state.chunkCount = 0
    const out = await retrieveKnowledge(db, 'acct', { embeddingsApiKey: 'sk-x', embeddingsBaseUrl: null }, 'q')
    expect(out).toEqual([])
  ...
  it('uses lexical FTS only when there is no embeddings key', async () => {
    ...
    const out = await retrieveKnowledge(db, 'acct', { embeddingsApiKey: null, embeddingsBaseUrl: null }, 'q')
  ...
  it('uses semantic search when an embeddings key is present', async () => {
    ...
    const out = await retrieveKnowledge(db, 'acct', { embeddingsApiKey: 'sk-x', embeddingsBaseUrl: null }, 'q', 3)
...
describe('ingestDocument', () => {
  it('embeds chunks when a key is present', async () => {
    const { db, state } = makeDb()
    await ingestDocument(db, 'acct', { embeddingsApiKey: 'sk-x', embeddingsBaseUrl: null }, 'doc-1', 'hello world')
    expect(h.embedTexts).toHaveBeenCalledTimes(1)
  ...
  it('stores chunks without embeddings when there is no key', async () => {
    const { db, state } = makeDb()
    await ingestDocument(db, 'acct', { embeddingsApiKey: null, embeddingsBaseUrl: null }, 'doc-1', 'hello world')
    expect(h.embedTexts).not.toHaveBeenCalled()
  ...
  it('deletes existing chunks and inserts nothing for empty content', async () => {
    const { db, state } = makeDb()
    await ingestDocument(db, 'acct', { embeddingsApiKey: 'sk-x', embeddingsBaseUrl: null }, 'doc-1', '   ')
    expect(state.deletedFor).toBe('doc-1')
  ...
  it('still stores lexical chunks when embedding fails, then rethrows', async () => {
    ...
    await expect(
      ingestDocument(db, 'acct', { embeddingsApiKey: 'sk-x', embeddingsBaseUrl: null }, 'doc-1', 'hello world'),
