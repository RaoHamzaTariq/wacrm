...
export async function POST() {
  try {
    ...
    const { key: embeddingsApiKey, corrupt, embeddingsBaseUrl } = await loadEmbeddingsKey(
      supabase,
      accountId,
    )
    ...
    for (const doc of docs ?? []) {
      try {
        await ingestDocument(supabase, accountId, { embeddingsApiKey, embeddingsBaseUrl }, doc.id, doc.content)
        reindexed += 1
      } catch (err) {
